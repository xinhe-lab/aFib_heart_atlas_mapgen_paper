...
save(odd_heatmap, file = "OR.rdata")

pdf('heatmap_final_notlog.pdf', width=5, height=3)
#odd_log <- log(odd_heatmap)
Heatmap(matrix = odd_heatmap, cluster_rows = F, 
        cluster_columns = F, show_row_names = T, row_names_side = "left",
        name = "Odd Ratio",
        col = circlize::colorRamp2(c(0,7,60), c("lightblue","white","firebrick")), 
...
save(odd_h3k, file = "odd_h3k.rdata")
#odd_h3k$Odd_ratio <- log(odd_h3k$Odd_ratio)
p <- ggplot(odd_h3k, aes(x=celltypes, y=Odd_ratio)) + 
...
ggsave(filename = "barplot_notlog.pdf", device = "pdf", plot = p, dpi = 600,width = 5, height = 3, limitsize = FALSE )
